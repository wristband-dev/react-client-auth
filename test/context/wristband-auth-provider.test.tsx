...
describe('WristbandAuthProvider', () => {
  ...
  it('clears token state when authentication changes', async () => {
    ...
    // Mock session fetch, then token fetch
    ...
    const TestTokenConsumer = () => {
      ...
      const handleClearAuth = () => context!.clearAuthData();
      const handleGetToken = async () => {
        try {
          const result = await context!.getToken();
          setToken(result || 'EMPTY_RESULT');
          setError('');
        } catch (err) {
    ...
    expect(apiClient.get).toHaveBeenCalledWith('/api/auth/session', requestOptions);

    // Get and cache a token
    await act(async () => screen.getByTestId('get-token').click());
    await waitFor(() => expect(screen.getByTestId('token').textContent).toBe('cached-token-123'));
    ...
    // Now try to get token again - this should fail since user is unauthenticated
    await act(async () => screen.getByTestId('get-token').click());
    await waitFor(() => expect(screen.getByTestId('error').textContent).toBe('User is not authenticated'));
    await waitFor(() => expect(screen.getByTestId('token').textContent).toBe('ERROR'));
  ...
  describe('getToken functionality', () => {
    ...
    it('retries on 5xx server errors and succeeds on second attempt', async () => {
      const mockTokenResponse = { accessToken: 'retry-success-token', expiresAt: Date.now() + 3600000 };
      const serverError = new ApiError('Server Error');
      serverError.status = 500;

      // First call for session, second for token (500 error), third for token (success)
      vi.mocked(apiClient.get)
        .mockResolvedValueOnce({ data: mockSessionData, status: 200, headers: new Headers() })
        .mockRejectedValueOnce(serverError) // First token attempt fails
        .mockResolvedValueOnce({ data: mockTokenResponse, status: 200, headers: new Headers() }); // Second attempt succeeds
      ...
      // Get token (should succeed after retry)
      await act(async () => screen.getByTestId('get-token').click());

      await waitFor(() => {
        expect(screen.getByTestId('token').textContent).toBe('retry-success-token');
        expect(screen.getByTestId('error').textContent).toBe('');
      });

      // Verify retry happened - session + failed token + successful token = 3 calls
      expect(apiClient.get).toHaveBeenCalledTimes(3);
    });

    it('retries 3 times on 5xx errors then fails', async () => {
      const serverError = new ApiError('Server Error');
      serverError.status = 503; // Service Unavailable

      // Session succeeds, then 3 token attempts all fail
      vi.mocked(apiClient.get)
        .mockResolvedValueOnce({ data: mockSessionData, status: 200, headers: new Headers() })
        .mockRejectedValue(serverError); // All subsequent calls fail
      ...
      // Try to get token (should fail after 3 retries)
      await act(async () => screen.getByTestId('get-token').click());

      await waitFor(() => {
        expect(screen.getByTestId('error').textContent).toBe('Failed to fetch token');
      });

      // Verify all 3 retry attempts were made - session + 3 token attempts = 4 calls
      expect(apiClient.get).toHaveBeenCalledTimes(4);
    });

    it('does not retry on 4xx client errors (403)', async () => {
      const clientError = new ApiError('Forbidden');
      clientError.status = 403;

      // Session succeeds, token fails with 403
      vi.mocked(apiClient.get)
        .mockResolvedValueOnce({ data: mockSessionData, status: 200, headers: new Headers() })
      ...
      // Try to get token (should fail immediately, no retries)
      await act(async () => screen.getByTestId('get-token').click());
      ...
      // Verify NO retries happened - only session + 1 token attempt = 2 calls
      expect(apiClient.get).toHaveBeenCalledTimes(2);
    });

    it('does not retry on 4xx client errors (404)', async () => {
      const clientError = new ApiError('Not Found');
      clientError.status = 404;

      // Session succeeds, token fails with 404
      vi.mocked(apiClient.get)
        .mockResolvedValueOnce({ data: mockSessionData, status: 200, headers: new Headers() })
        .mockRejectedValueOnce(clientError);
    ...
    it('retries on network errors and eventually succeeds', async () => {
      const mockTokenResponse = { accessToken: 'network-retry-token', expiresAt: Date.now() + 3600000 };
      const networkError = new Error('Network Error'); // No status code

      // Session succeeds, first two token attempts fail with network error, third succeeds
      vi.mocked(apiClient.get)
        .mockResolvedValueOnce({ data: mockSessionData, status: 200, headers: new Headers() })
        .mockRejectedValueOnce(networkError) // First token attempt fails
        .mockRejectedValueOnce(networkError) // Second token attempt fails
        .mockResolvedValueOnce({ data: mockTokenResponse, status: 200, headers: new Headers() }); // Third succeeds
      ...
      // Get token (should succeed after retries)
      await act(async () => screen.getByTestId('get-token').click());

      await waitFor(() => {
        expect(screen.getByTestId('token').textContent).toBe('network-retry-token');
        expect(screen.getByTestId('error').textContent).toBe('');
      });
  ...
  describe('fetchSession retry logic', () => {
    it('retries on 5xx server errors and succeeds on second attempt', async () => {
      ...
      vi.mocked(apiClient.get)
        .mockRejectedValueOnce(serverError) // First session attempt fails
        .mockResolvedValueOnce({ data: mockSessionData, status: 200, headers: new Headers() }); // Second attempt succeeds
      ...
      // Verify retry happened - 2 session attempts
      expect(apiClient.get).toHaveBeenCalledTimes(2);
      expect(apiClient.get).toHaveBeenCalledWith(defaultProps.sessionUrl, requestOptions);
    });
    ...
    it('does not retry on 4xx client errors (401) and redirects to login', async () => {
      // Mock console.log to prevent error output in tests
      console.log = vi.fn();

      const clientError = new ApiError('Unauthorized');
      clientError.status = 401;

      vi.mocked(apiClient.get).mockRejectedValueOnce(clientError);
    ...
    it('retries on network errors and eventually succeeds', async () => {
      ...
      vi.mocked(apiClient.get)
        .mockRejectedValueOnce(networkError) // First session attempt fails
        .mockRejectedValueOnce(networkError) // Second session attempt fails
        .mockResolvedValueOnce({ data: mockSessionData, status: 200, headers: new Headers() }); // Third succeeds
      ...
      // Verify retries happened - 2 failed + 1 successful = 3 calls
      expect(apiClient.get).toHaveBeenCalledTimes(3);
    });
    ...
    it('retries 3 times on network errors then redirects to logout', async () => {
      ...
      // All 3 session attempts fail with network error
    ...
    it('does not retry when disableRedirectOnUnauthenticated is true and 4xx error occurs', async () => {
      // Mock console.log to prevent error output in tests
      console.log = vi.fn();
    ...
    it('sets unauthenticated state when disableRedirectOnUnauthenticated is true and all retries fail', async () => {
      // Mock console.log to prevent error output in tests
      console.log = vi.fn();
